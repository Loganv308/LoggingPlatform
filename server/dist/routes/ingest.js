...
router.post('/', async (req, res) => {
    const { level, service, message, ts, metadata } = req.body;
    if (!level || !service || !message) {
        res.status(400).json({ error: 'level, service, and message are required' });
        return;
    }
    const result = await pool.query(`INSERT INTO logs (ts, level, service, message, metadata)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, ts`, [ts ?? new Date().toISOString(), level.toUpperCase(), service, message, metadata ?? null]);
...
router.post('/batch', async (req, res) => {
    ...
    await pool.query(`INSERT INTO logs (ts, level, service, message, metadata) VALUES ${values}`, params);
