...
router.get('/', async (req, res) => {
    const { service, level, search, since, until, limit = '200' } = req.query;
    const conditions = [];
    const params = [];
    if (service) {
        params.push(service);
    ...
    if (search) {
        params.push(`%${search}%`);
        conditions.push(`(message ILIKE $${params.length} OR service ILIKE $${params.length})`);
    }
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    params.push(Math.min(Number(limit), 1000));
    const result = await pool.query(`SELECT id, ts, level, service, message, metadata
     FROM logs
     ${where}
     ORDER BY ts DESC
